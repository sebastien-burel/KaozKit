// Copy of mlx-swift-lm's Libraries/MLXLLM/Models/Apertus.swift at the
// revision Package.swift pins (3e6ea1ed), registered over the package's own
// "apertus" in MLXChatActor. One difference, in XIELU and sanitize: the
// checkpoints mlx-lm converts store the XIELU alphas as scalars (its
// sanitizer squeezes them), and the package's copy expects [1], so every
// Apertus v1.5 conversion fails to load. Upstream fix proposed; delete this
// file and the registration once the pinned revision carries it.
...
public struct KaozApertusConfiguration: Codable, Sendable {
...
private class XIELU: Module, UnaryLayer {
    ...
    override public init() {
        // Scalars, as mlx-lm holds them; `sanitize` squeezes the [1] of the
        // original checkpoints to match.
...
private class ApertusAttention: Module {
    let args: KaozApertusConfiguration
    let scale: Float
    ...
    init(args: KaozApertusConfiguration) {
        self.args = args
...
private class ApertusBlock: Module {
    ...
    public init(_ args: KaozApertusConfiguration) {
...
public class KaozApertusModel: Module, LLMModel, KVCacheDimensionProvider {
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        ...
        for (key, value) in weights {
            ...
            if key.contains("self_attn.rotary_emb.inv_freq") {
                weights[key] = nil
            } else if key.hasSuffix("alpha_p") || key.hasSuffix("alpha_n") {
                // XIELU parameters: [1] in the original checkpoints, [] once
                // converted by mlx-lm. The layer holds scalars.
                weights[key] = value.squeezed()
            }
        }
        // The package's filterLMHeadWeights is not public: drop every tensor
        // of the absent head when the embedding doubles as the output.
        guard tieWordEmbeddings else { return weights }
...
extension KaozApertusModel: LoRAModel {
